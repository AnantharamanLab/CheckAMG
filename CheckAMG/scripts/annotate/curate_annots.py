...
emit_step_banner(title=f"Step {snakemake.params.step[0]}/{snakemake.params.step[1]}: Curate the predicted functions based on genomic context", log_path=log_file, append_raw=False)
...
EXC_LABEL_MAP = {
    "essential": "other non-auxiliary function",
    "glucan": "GH/GT",
...
def _pretty_filter_reason_token(tok: str, avg_array_len_limit: int | None, type_name: str) -> str:
    ...
    if tok == "below_min_amg_weight":
        return "AMG weight below minimum"
...
def summarize_annot_table(table, hmm_descriptions, viral_true_col):
    ...
    # Add missing columns and normalise dtype for all dist columns, since read_csv infers all-null columns as Utf8
    for c in dist_cols:
        if c not in table.columns:
            table = table.with_columns(pl.lit(None, dtype=pl.Float64).alias(c))

    table = table.with_columns(
        [pl.col(c).cast(pl.Float64, strict=False).alias(c) for c in dist_cols]
    )
...
def _ensure_context_cols(df: pl.DataFrame, viral_true_col: str) -> pl.DataFrame:
    ...
    viral_false_col = "protein_in_ambig_region"
...
def filter_flagged_annots(
    ...
    if isinstance(filter_presets, str):
        presets_raw = {p.strip().lower() for p in filter_presets.split(",") if p.strip()}
    else:
        presets_raw = {str(p).strip().lower() for p in (filter_presets or [])}

    def _bare(s: str) -> str:
        """Strip 'filter_' or 'allow_' prefix to get the bare exception stem."""
        for pfx in ("filter_", "allow_"):
            if s.startswith(pfx):
                return s[len(pfx):]
        return s

    # Normalize presets to bare stems so "allow_glucan", "filter_glucan", and "glucan" all match
    presets = {_bare(p) for p in presets_raw}

    cols = flagged_ids.columns
    filter_cols = sorted([c for c in cols if c.startswith("filter_")])
    exceptions = sorted({c[len("filter_"):] for c in filter_cols}) # bare stems already

    # rest of function: exc is already a bare stem, so `exc in presets` now works correctly
...
def filter_metabolism_annots_scored(
    table: pl.DataFrame,
    metab_ids: list[str],
    all_metab_ids: list[str],
    flagged_metabolism_table: pl.DataFrame,
    filter_presets,
) -> tuple[pl.DataFrame, pl.DataFrame]:
    """Filter metabolic annotations using the AMG weight-scored table plus keyword filters.

    Two-stage filter:

    1. **Weight gate** -- genes with any HMM annotation matching an AMG ID whose
       AMG_weight is below the threshold are rejected immediately, recorded in the
       audit as ``remove_reason = "below_min_amg_weight"``.
    2. **Keyword/flag filter** -- the weight-passing genes are then passed through
       ``filter_flagged_annots`` with the same flagged-ID table and filter presets
       used for physiology and regulation, producing the same audit columns.

    The returned audit concatenates both stages so ``log_filter_summary`` sees all
    removed rows and reports each reason (weight + keyword flags) the same way as
    before, plus the extra weight-specific count logged separately in ``main()``.

    Parameters
    ----------
    table:
        Full annotation table (all genes).
    metab_ids:
        AMG IDs that passed the AMG_weight threshold.
    all_metab_ids:
        All AMG IDs regardless of weight -- used to catch genes whose only AMG
        matches are below-threshold ones.
    flagged_metabolism_table:
        Flagged-ID filter table (same format as physiology/regulation).
    filter_presets:
        Active filter preset strings (same as physiology/regulation).
    """
    # All genes with any AMG annotation (weight-passing or not)
    candidates = table.filter(_category_mask_expr(table, all_metab_ids)).unique()

    # Stage 1: weight gate
    weight_pass = candidates.filter(_category_mask_expr(candidates, metab_ids))
    weight_fail = candidates.filter(~_category_mask_expr(candidates, metab_ids))

    audit_weight_fail = weight_fail.with_columns(
        [
            pl.lit(True).alias("removed"),
            pl.lit(False).alias("kept"),
            pl.lit("below_min_amg_weight").alias("remove_reason"),
            pl.lit(None, dtype=pl.Utf8).alias("keep_reason"),
        ]
    )

    # Stage 2: keyword/flag filter on weight-passing genes, identical to phys/reg
    weight_pass_filtered, audit_flag_filter = filter_flagged_annots(
        weight_pass, flagged_metabolism_table, metab_ids, filter_presets
    )
    weight_pass_filtered = weight_pass_filtered.unique()

    audit = pl.concat([audit_weight_fail, audit_flag_filter], how="diagonal")

    validate_audit_table(audit, "metabolism_filter_audit")
    return (
        weight_pass_filtered.sort(["Genome", "Contig", "gene_number"]),
        audit.sort(["Genome", "Contig", "gene_number"]),
    )
...
def main():
    ...
    min_amg_weight = _parse_float_or_none(snakemake.params.min_amg_weight)
    if min_amg_weight is None:
        min_amg_weight = 0.0
    ...
    input_table = as_parquet_path_if_enabled(snakemake.params.context_table, save_to_parquet)
    out_metabolism_table = as_parquet_path_if_enabled(snakemake.params.metabolism_table_out, save_to_parquet)
    out_metabolism_audit = as_parquet_path_if_enabled(snakemake.params.metabolism_table_audit, save_to_parquet)
    out_physiology_table = as_parquet_path_if_enabled(snakemake.params.physiology_table_out, save_to_parquet)
    out_physiology_audit = as_parquet_path_if_enabled(snakemake.params.physiology_table_audit, save_to_parquet)
    out_regulation_table = as_parquet_path_if_enabled(snakemake.params.regulation_table_out, save_to_parquet)
    out_regulation_audit = as_parquet_path_if_enabled(snakemake.params.regulation_table_audit, save_to_parquet)
    all_annot_out_table = as_parquet_path_if_enabled(snakemake.params.all_annot_out_table, save_to_parquet)
    ...
    metabolism_table = pl.read_csv(
        metabolism_ref,
        separator="\t",
        schema_overrides={"id": pl.Utf8, "db": pl.Utf8, "name": pl.Utf8, "AMG_weight": pl.Float64},
    )
    ...
    logger.debug(f"Filtering reference AMG table by AMG_weight >= {min_amg_weight}...")
    metabolism_table_filtered = metabolism_table.filter(
        pl.col("AMG_weight").cast(pl.Float64, strict=False).fill_null(0.0) >= min_amg_weight
    )
    logger.debug(f"Reference AMGs passing weight threshold ({min_amg_weight}): {metabolism_table_filtered.height:,} of {metabolism_table.height:,}")
    ...
    metab_ids = metabolism_table_filtered["id"].unique().to_list()
    all_metab_ids = metabolism_table["id"].unique().to_list()
    ...
    metabolism_table_out, metabolism_filter_audit = filter_metabolism_annots_scored(
        annot_table, metab_ids, all_metab_ids, flagged_metab_table, filter_presets
    )
    ...
    _n_below_weight = int(
        metabolism_filter_audit.filter(
            pl.col("remove_reason").cast(pl.Utf8, strict=False) == "below_min_amg_weight"
        ).height
    )
