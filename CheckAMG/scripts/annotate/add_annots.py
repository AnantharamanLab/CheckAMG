...
emit_step_banner(title=f"Step {snakemake.params.step[0]}/{snakemake.params.step[1]}: Merge functional annotations with protein metadata", log_path=log_file, append_raw=False)
...
def main():
    ...
    if save_to_parquet:
        gene_index_table = as_parquet_path_if_enabled(gene_index_table, save_to_parquet)
        gene_index_df = pl.read_parquet(gene_index_table)
        vscores = as_parquet_path_if_enabled(vscores, save_to_parquet)
        vscores_df = pl.read_parquet(vscores)
        filtered_hmm_results = as_parquet_path_if_enabled(filtered_hmm_results, save_to_parquet)
    ...
    if save_to_parquet:
        output = as_parquet_path_if_enabled(output, save_to_parquet)
