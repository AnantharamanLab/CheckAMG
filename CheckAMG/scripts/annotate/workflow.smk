...
if input_type == "nucl":
    ...
    rule filter_by_length:
        ...
        script:
            os.path.join(config["paths"]["steps_dir"], "filter_by_length.py")
    ...
    rule check_circular:
        ...
        script:
            os.path.join(config["paths"]["steps_dir"], "check_circular.py")
    ...
    rule filter_by_cds:
        ...
        script:
            os.path.join(config["paths"]["steps_dir"], "filter_by_cds.py")
...
rule assign_annots:
    ...
    output:
        touch(os.path.join(config["paths"]["output_dir"], "snakemake", "hmm_annotate.done"))
    params:
        ...
        metabolic_cutoff_file = os.path.join(config["paths"]["db_dir"], "METABOLIC_cutoffs.tsv"),
        keep_full_hmm_results = bool(config["keep_full_hmm_results"]),
        save_to_parquet = bool(config["save_to_parquet"]),
        debug = bool(config["debug"]),
        log = config["log"],
        step = (5,11) if input_type == "nucl" else (2,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "hmm_annotate.py")


# Obtain gene information from input (prodigal-formatted) .faa and genome information from...
rule index_genes:
    input:
        os.path.join(config["paths"]["output_dir"], "snakemake", "hmm_annotate.done")
    output:
    ...
    params:
        ...
        debug = bool(config["debug"]),
        log = config["log"],
        step = (6,11) if input_type == "nucl" else (3,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "map_protein_data.py")
...
rule add_annots:
    ...
    params:
        ...
        debug = bool(config["debug"]),
        log = config["log"],
        step = (7,11) if input_type == "nucl" else (4,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "add_annots.py")
...
rule genome_context:
    ...
    params:
        ...
        debug = bool(config["debug"]),
        log = config["log"],
        step = (8,11) if input_type == "nucl" else (5,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "genome_context.py")
...
rule curate_annots:
    ...
    params:
        ...
        debug = bool(config["debug"]),
        log = config["log"],
        step = (9,11) if input_type == "nucl" else (6,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "curate_annots.py")
...
rule organize_proteins:
    ...
    params:
        ...
        debug = bool(config["debug"]),
        log = config["log"],
        step = (10,11) if input_type == "nucl" else (7,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "organize_proteins.py")
...
rule make_final_table:
    ...
    params:
        all_genes_annotated = os.path.join(config["paths"]["output_dir"], "results", "gene_annotations.tsv"),
        gene_index = os.path.join(config["paths"]["output_dir"], "wdir", "gene_index.tsv"),
        metabolic_categories_ref = os.path.join(config["paths"]["files_dir"], "AMGs_categorized.tsv"),
        physiology_categories_ref = os.path.join(config["paths"]["files_dir"], "APGs_categorized.tsv"),
        regulation_categories_ref = os.path.join(config["paths"]["files_dir"], "AReGs_categorized.tsv"),
        metabolism_table = os.path.join(config["paths"]["output_dir"], "results", "metabolic_genes_curated.tsv"),
        metabolism_categories_out = os.path.join(config["paths"]["output_dir"], "results", "metabolic_gene_categories.tsv"),
        physiology_table = os.path.join(config["paths"]["output_dir"], "results", "physiology_genes_curated.tsv"),
        physiology_categories_out = os.path.join(config["paths"]["output_dir"], "results", "physiology_gene_categories.tsv"),
        regulation_table = os.path.join(config["paths"]["output_dir"], "results", "regulation_genes_curated.tsv"),
        regulation_categories_out = os.path.join(config["paths"]["output_dir"], "results", "regulation_gene_categories.tsv"),
        final_table = os.path.join(config["paths"]["output_dir"], "results", "final_results.tsv"),
        save_to_parquet = bool(config["save_to_parquet"]),
        debug = bool(config["debug"]),
        log = config["log"],
        step = (11,11) if input_type == "nucl" else (8,8)
    threads:
        config["threads"]
    resources:
        mem = config["mem_limit"]
    ...
    script:
        os.path.join(config["paths"]["steps_dir"], "make_final_tables.py")