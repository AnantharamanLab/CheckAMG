...
from CheckAMG.scripts.denovo.anchored_view import AnchoredView
from CheckAMG.scripts.denovo.constants import TRUE_AVG_IDX

if TYPE_CHECKING:
    from CheckAMG.scripts.denovo.model import CheckAMGPST


def opposite_class_priority_table(device: torch.device | None = None) -> torch.Tensor:
    # row = anchor class (0-3); cols = opposite classes ordered most-opposite first.
    # rule: p1 flips viral keeps AVG, p2 keeps viral flips AVG, p3 flips both (class = 2*AVG + viral)
    return torch.tensor(
...
def restrict_to_priority_class(
    candidate_mask: torch.Tensor,
    class_labels: torch.Tensor,
    anchor_class: torch.Tensor,
    table: torch.Tensor,
) -> torch.Tensor:
    # candidate_mask [P, N] flags different-class candidates per anchor row; returns the same
    # mask narrowed to only the highest-priority opposite class that has >=1 candidate in that row.
    # rows with no candidate are returned all-False so the caller can apply its own fallback.
    num_rows = candidate_mask.shape[0]
    cand_class = class_labels.unsqueeze(0)
    priorities = table[anchor_class]

    result = torch.zeros_like(candidate_mask)
    assigned = torch.zeros(num_rows, dtype=torch.bool, device=candidate_mask.device)
    for level in range(priorities.shape[1]):
        target = priorities[:, level].unsqueeze(1)
        in_target = candidate_mask & (cand_class == target)
        take = in_target.any(dim=1) & (~assigned)
        result[take] = in_target[take]
...
class TripletLoss(torch.nn.Module):
    def __init__(
        ...
        positive_mining_strategy: Literal["all", "hard", "esm_easy"] = "esm_easy",
        ...
        self.opposite_class_negatives = opposite_class_negatives
    ...
    def _mine_negative(
        self,
        triplet_margin: torch.Tensor,
        same_label: torch.Tensor,
        class_labels: OptTensor = None,
        anchor_class: OptTensor = None,
    ) -> torch.Tensor:
        # shape: [AP-pairs, num_proteins]
        meets_condition = self._negative_mining_condition(triplet_margin, same_label)

        # restrict the hard/semihard candidate set to the most-opposite class present per anchor
        if self.opposite_class_negatives:
            table = opposite_class_priority_table(device=triplet_margin.device)
            meets_condition = restrict_to_priority_class(
                meets_condition, class_labels, anchor_class, table
            )
        ...
        if no_neg.any():
            if self.opposite_class_negatives:
                # keep the opposite-class priority for the relaxed fallback too
                fallback = restrict_to_priority_class(
                    ~same_label,
                    class_labels,
                    anchor_class,
                    opposite_class_priority_table(device=triplet_margin.device),
                )
                mask = no_neg.unsqueeze(-1) & fallback
            else:
    ...
    @torch.no_grad()
    def _esm_easy_positive_mining(
        self, esm_dist: torch.Tensor, same_label: torch.Tensor
    ) -> PairTensor:
        # easy positive: the nearest same-class protein in ESM space (positives are mined in the
        # input ESM embedding space, matching the original PST design, but made supervised by class)
        masked_dist = (
            # mask other classes
            esm_dist.masked_fill(~same_label, torch.inf)
            # mask self
            .fill_diagonal_(torch.inf)
        )

        anchor_idx = torch.arange(esm_dist.shape[0], device=esm_dist.device)
        pos_idx = masked_dist.argmin(-1)

        has_pos_pair = torch.isfinite(masked_dist).any(-1)
    ...
    def _mine_triplets(
        ...
        for start_idx in range(0, num_pairs, subset_size):
            ...
            subset_neg_idx = self._mine_negative(
                ...
                anchor_class=class_labels[subset_anchor_idx],
            )
    ...
    def forward(
        ...
        if self.positive_mining_strat == "esm_easy":
            if esm_dist is None:
                raise ValueError(
                    "The 'esm_easy' positive mining strategy requires `esm_dist` "
                    "(pairwise distances in the input ESM embedding space)."
                )
            all_anchor_idx, all_pos_idx = self._esm_easy_positive_mining(
                esm_dist, same_label
            )
        elif self.positive_mining_strat == "hard":
